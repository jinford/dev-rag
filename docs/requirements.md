...
本システムは、社内の Git リポジトリやドキュメントを対象に、
...
1. 開発者が CLI または REST API で Wiki 生成を実行する。
   - CLI: `dev-rag wiki generate --repo myapp`
   - API: `POST /api/v1/repos/myapp/wiki`
2. システムは Postgres から対象リポジトリ・スナップショットのチャンクを検索する。
3. ページ種別（概要、アーキテクチャ、機能詳細など）ごとに関連するチャンクを取得し、LLM に渡して Markdown を生成する。
4. `/var/lib/dev-rag/wikis/<リポジトリ名>/` ディレクトリにファイルとして出力する。
5. 生成されるページには以下を含む：
   - リポジトリ概要と目次
...
2. ダッシュボードには、インデックス済みのリポジトリ一覧が表示される。
3. リポジトリを選択すると、生成された Wiki が表示される。
4. Wiki は Markdown がレンダリングされ、Mermaid 図も表示される。
5. サイドバーから異なるページ（index.md, architecture.md, features/ 配下など）に遷移できる。
6. 「Wiki を再生成」ボタンをクリックすると、バックグラウンドで Wiki が更新される。
...
- システムは、対象とする Git リポジトリの情報（名前、URL、デフォルトブランチ）を登録・更新できなければならない。
- 各リポジトリには一意な ID が付与される。
- 将来的に複数リポジトリを横断した検索を行う可能性を考慮し、リポジトリ単位の識別は必須とする。
- リポジトリ情報は PostgreSQL 上の専用テーブルに永続化し、CLI から参照・更新できるようにする。
...
- システムは、`リポジトリ ID + コミットハッシュ` の組み合わせでスナップショットを管理しなければならない。
- 各スナップショットに対して、以下の情報を保持する。
  ...
  - インデックス完了日時
- 同じコミットハッシュに対して複数回インデックスを実行した場合も、重複したスナップショットは作成せず、既存レコードを更新する。

### 8.3 ファイル・チャンク管理機能

- システムはスナップショット内のファイル一覧を取得し、以下の情報を保存できなければならない。
  - ファイルパス
  - ファイルサイズ
  - 言語種別（簡易な判定でよい）
  - ファイル内容のハッシュ
...
- `.gitignore` にマッチするファイルはスキップし、追加でリポジトリルートに `.devragignore` を置くことで明示的に除外パターンを定義できるようにする。

### 8.4 Embedding 生成・保存機能
...
- デフォルトでは、同一リポジトリ・同一参照（ブランチやタグ）で最後に成功したスナップショットとの比較を行い、差分があれば差分更新、なければスキップする。
...
- 同一リポジトリかつ同一参照を対象とするインデックス処理は、PostgreSQL のアドバイザリロック等を用いて排他制御し、同時実行時はジョブを順番待ちさせる。

### 8.6 ベクトル検索機能（内部 API）

- システムは、以下の条件を入力としてベクトル検索を実行できなければならない。
  - リポジトリ名（または ID）
  - スナップショット（コミットハッシュまたはブランチ名）
  - 検索クエリ文字列
  - 返却件数（limit）
  - パス prefix（任意）
  - ソース種別（任意）
...
- システムは上記入力をもとに、以下の処理を行う。
  ...
  2. スナップショットおよびフィルタ条件で対象チャンクを制限する。
  3. pgvector の距離演算（cosine 等）を用いてクエリベクトルとの距離を計算し、類似度の高いチャンクを上位から取得する。
  4. オプションに応じて、ヒットチャンクの前後チャンクも取得し、コンテキストとして返却する。
- 戻り値には、少なくとも次の情報を含める。
  - ファイルパス
  - 開始行・終了行
  - チャンクテキスト
...
- システムは、対象リポジトリおよび参照（コミット or ブランチ）を指定して、
  以下のようなページ構成の Wiki を Markdown 形式で生成できなければならない。
  - `index.md`: リポジトリ概要、目次、主要コンポーネントサマリー、最新コミット情報。
  - `architecture.md`: システム全体構成、依存関係、メッセージフローを説明し、Mermaid 図（例：flowchart, sequence）を最低 1 つ含む。
  - `features/<feature-name>.md`: 各機能ごとの責務、主なファイルを説明し、可能な限り機能内のフローを表した Mermaid 図を挿入する。
...
- 生成された Wiki は `/var/lib/dev-rag/wikis/<リポジトリ名>/` ディレクトリ配下に Markdown ファイルとして出力する。
...
- API のエンドポイントには、以下を含む。
  - `POST /api/v1/repos/:name/index`: インデックス更新をトリガー（非同期）
  - `POST /api/v1/repos/:name/wiki`: Wiki 生成をトリガー（非同期）
  - `GET /api/v1/jobs/:jobID`: ジョブステータス確認
  - `GET /api/v1/repos`: リポジトリ一覧取得
  - `GET /api/v1/repos/:name`: リポジトリ詳細取得
  - `GET /api/v1/repos/:name/wiki`: Wiki 情報取得
  - `GET /api/v1/repos/:name/wiki/files`: Wiki ファイル一覧取得
  - `GET /api/v1/repos/:name/wiki/files/:path`: Wiki ファイル取得（Markdown）
- JSON のキー名は Go 命名規約に準拠し、頭字語は大文字（`jobID`, `repoURL`）、それ以外はキャメルケース（`repoName`, `commitHash`）とする。
...
- データモデルは、コード以外のドキュメント（設計書、Runbook、チケットのテキストなど）を追加できるように設計する。
...
3. **対象データの拡大**
   - PDF / Office ファイルをテキスト・Markdown に変換して取り込む。
   - チケットシステム（例：Redmine）の内容を取り込み、障害対応のナレッジとして活用する。
