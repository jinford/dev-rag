...
type WikiGenerator struct {
	indexingReader domain.IndexingDataReader
...
func (g *WikiGenerator) GenerateArchitecturePage(
	...
	// 1. プロダクトの全ソースを取得
	sources, err := g.indexingReader.ListSourcesByProduct(ctx, productID)
	if err != nil {
	...
	for _, source := range sources {
		snapshot, err := g.indexingReader.GetLatestIndexedSnapshot(ctx, source.ID)
		if err != nil {
	...
	searchResult, err := g.searcher.Search(ctx, searchParams)
	if err != nil {
		log.Printf("警告: RAG検索に失敗しました: %v", err)
		searchResult = &domain.SearchResponse{Results: []*domain.SearchResult{}}
	} else {
		log.Printf("RAG検索結果: %d件", len(searchResult.Results))
	}

	// 5. プロンプト構築（階層的コンテキスト）
	prompt := g.promptBuilder.BuildArchitecturePrompt(
		architectureSummaries,  // 最優先コンテキスト
		searchResult.Results,   // 補足コンテキスト
		pseudoQuery,
	)
...
func (g *WikiGenerator) GenerateDirectoryPage(
	...
	// 1. 最新スナップショットを取得
	snapshot, err := g.indexingReader.GetLatestIndexedSnapshot(ctx, sourceID)
	if err != nil {
	...
	// uuid.UUIDをpgtype.UUIDに変換
	var pgSnapshotID pgtype.UUID
	if err := pgSnapshotID.Scan(snapshot.ID); err != nil {
		return fmt.Errorf("failed to convert snapshot ID: %w", err)
	}
