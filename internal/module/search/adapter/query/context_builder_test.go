...
func TestBuildContextWithHierarchy(t *testing.T) {
	// テストデータの準備
	parentChunk := &searchdomain.ChunkContext{
		ID:        uuid.New(),
	...
	childChunk1 := &searchdomain.ChunkContext{
		ID:        uuid.New(),
	...
	childChunk2 := &searchdomain.ChunkContext{
		ID:        uuid.New(),
	...
	results := []*EnhancedSearchResult{
		{
			SearchResult: &searchdomain.SearchResult{
				ChunkID:   uuid.New(),
				FilePath:  "test.go",
				StartLine: 6,
				EndLine:   10,
			...
			ChildChunks: []*searchdomain.ChunkContext{childChunk1, childChunk2},
		},
	}
