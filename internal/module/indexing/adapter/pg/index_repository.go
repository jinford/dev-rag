...
func (rw *IndexRepositoryRW) CreateFile(ctx context.Context, snapshotID uuid.UUID, path string, size int64, contentType string, contentHash string, language *string, domain *string) (*domain.File, error) {
...
func (r *IndexRepositoryR) GetFileByID(ctx context.Context, id uuid.UUID) (*domain.File, error) {
...
func (r *IndexRepositoryR) ListFilesBySnapshot(ctx context.Context, snapshotID uuid.UUID) ([]*domain.File, error) {
...
func (rw *IndexRepositoryRW) CreateChunk(ctx context.Context, fileID uuid.UUID, ordinal int, startLine int, endLine int, content string, contentHash string, tokenCount int, metadata *domain.ChunkMetadata) (*domain.Chunk, error) {
...
func (rw *IndexRepositoryRW) BatchCreateChunks(ctx context.Context, chunks []*domain.Chunk) error {
	if len(chunks) == 0 {
		return nil
	}
...
func (r *IndexRepositoryR) GetChunkByID(ctx context.Context, id uuid.UUID) (*domain.Chunk, error) {
...
func (r *IndexRepositoryR) ListChunksByFile(ctx context.Context, fileID uuid.UUID) ([]*domain.Chunk, error) {
...
func (r *IndexRepositoryR) GetChunkContext(ctx context.Context, chunkID uuid.UUID, beforeCount int, afterCount int) ([]*domain.Chunk, error) {
...
func (rw *IndexRepositoryRW) BatchCreateEmbeddings(ctx context.Context, embeddings []*domain.Embedding) error {
	if len(embeddings) == 0 {
		return nil
	}
...
// Note: SearchByProduct と SearchBySource は search モジュールの SearchRepository に移動されました

// === Private helpers ===

func convertSQLCFile(row sqlc.File) *domain.File {
	return &domain.File{
...
func convertSQLCSnapshotFile(row sqlc.SnapshotFile) *domain.SnapshotFile {
...
func convertSQLCChunk(row sqlc.Chunk) *domain.Chunk {
	return &domain.Chunk{
		ID:          PgtypeToUUID(row.ID),
...
func (r *IndexRepositoryR) GetChildChunks(ctx context.Context, parentID uuid.UUID) ([]*domain.Chunk, error) {
...
func (r *IndexRepositoryR) GetParentChunk(ctx context.Context, chunkID uuid.UUID) (*domain.Chunk, error) {
...
func (r *IndexRepositoryR) GetChunkTree(ctx context.Context, rootID uuid.UUID, maxDepth int) ([]*domain.Chunk, error) {
...
func (r *IndexRepositoryR) GetFilesByDomain(ctx context.Context, snapshotID uuid.UUID, domainName string) ([]*domain.File, error) {
	rows, err := r.q.GetFilesByDomain(ctx, sqlc.GetFilesByDomainParams{
		SnapshotID: UUIDToPgtype(snapshotID),
		Domain:     StringToNullableText(domainName),
	})
	if err != nil {
...
func (rw *IndexRepositoryRW) CreateSnapshotFile(ctx context.Context, snapshotID uuid.UUID, filePath string, fileSize int64, domainName *string, indexed bool, skipReason *string) (*domain.SnapshotFile, error) {
	sf, err := rw.q.CreateSnapshotFile(ctx, sqlc.CreateSnapshotFileParams{
		SnapshotID: UUIDToPgtype(snapshotID),
		FilePath:   filePath,
		FileSize:   fileSize,
		Domain:     StringPtrToPgtext(domainName),
		Indexed:    indexed,
...
func (r *IndexRepositoryR) GetSnapshotFilesBySnapshot(ctx context.Context, snapshotID uuid.UUID) ([]*domain.SnapshotFile, error) {
...
// GetBySnapshot は GetSnapshotFilesBySnapshot のエイリアスです（domain.SnapshotFileReader 実装）
func (r *IndexRepositoryR) GetBySnapshot(ctx context.Context, snapshotID uuid.UUID) ([]*domain.SnapshotFile, error) {
	return r.GetSnapshotFilesBySnapshot(ctx, snapshotID)
}

// GetDomainCoverageStats はドメイン別のカバレッジ統計を取得します
func (r *IndexRepositoryR) GetDomainCoverageStats(ctx context.Context, snapshotID uuid.UUID) ([]*domain.DomainCoverage, error) {
	...
	coverages := make([]*domain.DomainCoverage, 0, len(rows))
	for _, row := range rows {
		coverages = append(coverages, &domain.DomainCoverage{
			Domain:           row.Domain,
