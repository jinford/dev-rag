...
func TestFileSummaryGenerator_Generate(t *testing.T) {
	tests := []struct {
		...
		mockErr        error
		expectedErr    bool
		validateResult func(*testing.T, *domain.FileSummaryResponse)
	}{
		{
			name: "正常なレスポンス",
			req: domain.FileSummaryRequest{
			...
			expectedErr: false,
			validateResult: func(t *testing.T, resp *domain.FileSummaryResponse) {
...
func TestGenerateSummaryText(t *testing.T) {
	tests := []struct {
		name     string
		resp     *domain.FileSummaryResponse
		contains []string
	}{
		{
			name: "完全なサマリーテキスト",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			text := domain.GenerateSummaryText(tt.resp)
...
func TestFileSummaryResponse_JSON(t *testing.T) {
	...
	original := &domain.FileSummaryResponse{
		PromptVersion: "1.1",
		Summary: []string{
			"項目1",
			"項目2",
	...
	// デシリアライズ
	var restored domain.FileSummaryResponse
