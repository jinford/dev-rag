...
func (r *Ranker) AdjustRanking(ctx context.Context, results []*searchdomain.SearchResult) ([]*RankedResult, error) {
	if len(results) == 0 {
...
func (r *Ranker) FilterByLatestOnly(ctx context.Context, results []*searchdomain.SearchResult) ([]*searchdomain.SearchResult, error) {
	if len(results) == 0 {
		return []*searchdomain.SearchResult{}, nil
	}

	filtered := make([]*searchdomain.SearchResult, 0)
