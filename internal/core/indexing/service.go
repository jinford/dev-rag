...
type IndexService struct {
	...
	embedder        Embedder
	llmClient       wiki.Client // オプショナル
	chunkerFactory  chunker.ChunkerFactory
	languageDetect  chunker.LanguageDetector
...
func NewIndexService(cfg IndexServiceConfig) *IndexService {
	if cfg.ChunkerConfig == nil {
		cfg.ChunkerConfig = chunker.DefaultChunkerConfig()
	}
	if cfg.Logger == nil {
...
// convertChunkMetadata は chunker.ChunkMetadata を indexing.ChunkMetadata に変換する
func (s *IndexService) convertChunkMetadata(meta *chunker.ChunkMetadata) *ChunkMetadata {
	if meta == nil {
		return nil
	}

	// chunker.ChunkMetadata を indexing.ChunkMetadata に変換
	// chunker.ChunkMetadata には UpdatedAt がないため、nil を設定
	return &ChunkMetadata{
		...
		Author:               meta.Author,
		UpdatedAt:            nil, // chunker.ChunkMetadata にはこのフィールドがない
