...
func NewHierarchicalSearcher(indexRepo *repository.IndexRepositoryR, logger *slog.Logger) *HierarchicalSearcher {
	if indexRepo == nil {
		panic("hierarchical_search.NewHierarchicalSearcher: indexRepo is nil")
	}
	if logger == nil {
		logger = slog.Default()
	}
...
func (h *HierarchicalSearcher) EnrichWithHierarchy(
	...
	for _, result := range results {
		...
		if options.IncludeParent || options.IncludeAncestors {
			parent, err := h.indexRepo.GetParentChunk(ctx, result.ChunkID)
			if err != nil {
		...
		if options.IncludeChildren {
			children, err := h.indexRepo.GetChildChunks(ctx, result.ChunkID)
			if err != nil {
...
func (h *HierarchicalSearcher) GetParentChunk(ctx context.Context, chunkID uuid.UUID) (*models.Chunk, error) {
	parent, err := h.indexRepo.GetParentChunk(ctx, chunkID)
	if err != nil {
...
func (h *HierarchicalSearcher) GetChildChunks(ctx context.Context, chunkID uuid.UUID) ([]*models.Chunk, error) {
	children, err := h.indexRepo.GetChildChunks(ctx, chunkID)
	if err != nil {
...
func (h *HierarchicalSearcher) GetAncestors(ctx context.Context, chunkID uuid.UUID, maxDepth int) ([]*models.Chunk, error) {
	return h.getAncestors(ctx, chunkID, maxDepth)
}

// getAncestors は祖先チャンクを再帰的に取得する内部実装です
func (h *HierarchicalSearcher) getAncestors(ctx context.Context, chunkID uuid.UUID, maxDepth int) ([]*models.Chunk, error) {
	ancestors := make([]*models.Chunk, 0)
	...
	for {
		...
		// 親チャンクを取得
		parent, err := h.indexRepo.GetParentChunk(ctx, currentID)
		if err != nil {
			return nil, fmt.Errorf("failed to get parent chunk: %w", err)
		}
...
func (h *HierarchicalSearcher) GetChunkTree(ctx context.Context, rootID uuid.UUID, maxDepth int) ([]*models.Chunk, error) {
	tree, err := h.indexRepo.GetChunkTree(ctx, rootID, maxDepth)
	if err != nil {
...
func (h *HierarchicalSearcher) getChunkLabel(chunk *models.Chunk) string {
