...
// NewOpenAIClient は新しいOpenAIClientを作成する
...
// NewOpenAIClientWithModel はモデルを指定してOpenAIClientを作成する
func NewOpenAIClientWithModel(model string) (*OpenAIClient, error) {
...
// レート制限エラー時はExponential Backoffでリトライする
// JSON解析エラー時は最大1回リトライする
		...
		if req.ResponseFormat == "json" {
			if !isValidJSON(resp.Content) {
				...
				if jsonParseRetries > JSONParseMaxRetries {
					return CompletionResponse{}, fmt.Errorf("%w: JSON parse failed after %d retries", ErrInvalidResponseFormat, JSONParseMaxRetries)
				}
				// JSON解析に失敗した場合は再試行
				continue
			}
...
func (c *OpenAIClient) generateWithRetry(ctx context.Context, model string, req CompletionRequest) (CompletionResponse, error) {
	var lastErr error
	...
	// 最大リトライ回数を超過
	return CompletionResponse{}, fmt.Errorf("%w: %v", ErrMaxRetriesExceeded, lastErr)
}
