...
func TestFileSummaryGenerator_Integration(t *testing.T) {
	...
	tests := []struct {
		...
		validate    func(*testing.T, *FileSummaryResponse)
		description string
	}{
		{
...
func (u *User) Validate() error {
	...
	return nil
			...
			validate: func(t *testing.T, resp *FileSummaryResponse) {
				// サマリーが存在すること
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// サマリーテキストを生成して表示
			summaryText := GenerateSummaryText(resp)
...
func (c *Chunker) Chunk(content string, contentType string) ([]*models.Chunk, error) {
	switch contentType {
	case "code":
...
func (c *Chunker) chunkCode(content string) ([]*models.Chunk, error) {
	// AST解析を使用してチャンク化
...
func (c *Chunker) chunkMarkdown(content string) ([]*models.Chunk, error) {
	lines := strings.Split(content, "\n")
	chunks := make([]*models.Chunk, 0)
...
func (c *Chunker) chunkText(content string) ([]*models.Chunk, error) {
	// 段落単位で分割
